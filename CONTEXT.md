...
- Environment Setup probes all package rows through one structured hidden command. Python readiness is capability-based (`python` and `python3` resolve to the same target, `python3 -m venv`, and `python3 -m pip`); Debian status parsing accepts the exact `dpkg-query` value `install ok installed`. Missing or malformed probe markers project as `UNKNOWN`/'Unable to detect' rather than an incorrect `NOT_INSTALLED` state.
